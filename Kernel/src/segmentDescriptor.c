...
{
	printf("Base: %x Limit: %x %s LV%d %s %s %s %s %s",
			segmentGetBase(sd), segmentGetLimit(sd),
			segmentGetAccess(sd, SD_GET_S)? "Code/Data" : "System",
			segmentGetAccess(sd, SD_GET_LV) >> 5,
			segmentGetAccess(sd, SD_GET_P)? "P" : "NP",
			segmentGetAttrib(sd, SD_GET_AVL)? "AVL" : "NAVL",
			segmentGetAttrib(sd, SD_GET_L)? "L" : "NL",
			segmentGetAttrib(sd, SD_GET_DB)? "32B" : "16B",
			segmentGetAttrib(sd, SD_GET_G)? "G" : "NG");

	if (segmentGetType(sd, SD_CODE))
	{
		printf("C:%s%s%s\n",
			segmentGetType(sd, SD_CONFORMING)? "C" : "-",
			segmentGetType(sd, SD_EXECUTE_READ)? "R" : "-",
			segmentGetType(sd, SD_ACCESSED)? "A" : "-");
	}
	else
	{
		printf("D:%s%s%s\n",
			segmentGetType(sd, SD_EXPAND_DOWN)? "E" : "-",
			segmentGetType(sd, SD_WRITABLE)? "W" : "-",
			segmentGetType(sd, SD_ACCESSED)? "A" : "-");
	}
}
